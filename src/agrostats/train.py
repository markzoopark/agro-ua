...
TRAIN_LANG = {
    "uk": {
        ...
        "shap_title": "SHAP summary — {crop} ({model})",
    },
    "en": {
...
def plot_shap_summary(
    ...
    shap_subset = shap.Explanation(
        values=shap_values.values[:, [X_test.columns.get_loc(col) for col in top_features]],
        base_values=shap_values.base_values,
        data=X_test[top_features].to_numpy(),
    ...
    fig_dir = FIGURES_DIR / language
    fig_dir.mkdir(parents=True, exist_ok=True)
    plt.savefig(fig_dir / f"shap_{model_name}_{slugify(crop)}_{scenario}.png", dpi=220, bbox_inches="tight")
    plt.close()
